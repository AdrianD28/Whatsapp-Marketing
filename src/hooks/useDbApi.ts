...
export function useDbApi(creds: ApiCredentials | null) {
  ...
  const headers: Record<string, string> = accountKey
    ? { 'X-Account-Key': accountKey, 'Content-Type': 'application/json' }
    : { 'Content-Type': 'application/json' };

  const getLists = useCallback(async () => {
    if (!accountKey) return [] as any[];
  ...
  const createList = useCallback(async (name: string) => {
    if (!accountKey) throw new Error('No account');
